...
EmailCategoryType = Literal[
    "NEWSLETTERS", "PROMOTIONS", "PERSONAL", "GITHUB",
...
class EmailResponse(BaseModel):
    ...
    body: str = Field(..., description="Full draft email body to save")
...
class CategorizationBatch(BaseModel):
    """Wrapper model for multiple categorized items."""
    items: List[SimpleCategorizedEmail] = Field(
        ..., description="List of categorized emails"
    )

class OrganizationBatch(BaseModel):
    """Wrapper model for multiple organized results."""
    items: List[OrganizedEmail] = Field(
        ..., description="List of organization results"
    )

class CleanupBatch(BaseModel):
    """Wrapper model for multiple cleanup decisions."""
    items: List[EmailCleanupInfo] = Field(
        ..., description="List of cleanup decisions"
    )

class ResponsesBatch(BaseModel):
    """Wrapper model for multiple email responses."""
    items: List[EmailResponse] = Field(
        ..., description="List of responses to generate"
    )
